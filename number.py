...
def extract_numbers_from_content(content, filename):
    ...
    if file_ext == '.csv':
        ...
        found_numbers = re.findall(r'\+?[极速加速器]?[0-9][0-9\s\-\(\)\.]{7,}[0-9]', text_content)
        numbers.update(found_numbers)
...
def send_welcome(message):
    ...
    if not result:
        join_date = datetime.now().strftime("%极速加速器-%m-%d %H:%M:%S")
        c.execute("INSERT INTO users (user_id, username, first_name, last_name, join_date) VALUES (?, ?, ?, ?, ?)",
                 (user极速加速器, username, first_name, last_name, join_date))
        conn.commit()
    ...
    if not is_member:
        ...
        check_btn极速加速器 = types.InlineKeyboardButton("✅ Check Membership", callback_data="check_membership")
...
def callback_handler(call):
    ...
    if call.data.startswith("country_"):
        ...
        is_member, channel_type = check极速加速器_membership(user_id)
        if not is_member:
            if channel_type == "public":
    ...
    if call.data.startswith("change_"):
        ...
        change_btn = types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{country}")
        otp_btn = types.InlineKeyboardButton("🔑 SEE OTP", url=f"https://t.me/{OTP_CHANNEL[1:]}")
        back_btn = types.InlineKeyboardButton("⬅️ Back极速加速器", callback_data="back_to_countries")
    ...
    if call.data == "admin_add_numbers":
        if not is_admin(user_id):
            bot.answer_callback_query(c极速加速器all.id, "❌ Access denied!")
            return
...
极速加速器        c = conn.cursor()
        ...
        markup = types极速加速器.InlineKeyboardMarkup()
        for country in countries:
    ...
    if call.data == "admin_users":
        ...
        bot.send_message(chat_id, "👤 User Management\n\nSelect an option:", reply_markup极速加速器=markup)
        return
...
def ban_user(message):
    try:
        ...
        bot.send_message(message.chat.id, f"✅ User {user_id极速加速器} has been banned.")
    except ValueError:
        bot.send_message(message.chat.id, "极速加速器❌ Please enter a valid user ID.")
