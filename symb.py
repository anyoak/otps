...
"""
        ...
        success_msg = await context.bot.send_message(target_id, "🎉 *WELCOME ABOARD!*", parse_mode=ParseMode.MARKDOWN)
        welcome_frames = [
...
def main():
    ...
    broadcast_conv = ConversationHandler(
        ...
        states={
            WAITING_FOR_BROADCAST: [MessageHandler(filters.ALL, on_broadcast_content)]
        },
    ...
    application.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & filters.User(user_ids=list(captcha_store.keys())), 
        answer_captcha
    ))
