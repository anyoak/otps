...
async def send_loading_sequence(chat_id: int):
    ...
    try:
        ...
        msg2 = await bot.send_message(chat_id, "🔍 *VERIFYING DATABASE ACCESS*")
        verification_steps = [
        ...
        msg3 = await bot.send_message(chat_id, "🛡️ *SECURITY SCAN IN PROGRESS*")
        security_frames = [
...
async def send_success_animation(chat_id: int, user_name: str):
    ...
    success_msg = await bot.send_message(chat_id, "🎉 *WELCOME ABOARD!*")
...
async def on_broadcast_content(message: types.Message, state: FSMContext):
    ...
    progress_msg = await message.answer(f"📊 *Broadcast Progress:* 0/{total_users}")
